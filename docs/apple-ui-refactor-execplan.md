...
- 完成后的可观察行为：
  ...
  - Apple 风格服务页可搜索并按自适应网格分组；时间、加载、缓存、刷新、空状态、警告和错误得到一致反馈。
...
- Apple 服务页增加搜索、自适应分组和系统图标；角色插画只用于品牌、加载与空状态。
...
- Decision：Apple 风格建立在 Material3/Compose 和现有依赖上。
  - Context：不应引入 Apple 私有资产或高成本实时模糊。
  - Chosen approach：系统图标、Material 语义、色调表面和 Compose 原生动画/手势。
