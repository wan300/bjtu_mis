...
export async function buildServer(options: { config?: PlatformConfig; db?: Database } = {}) {
  ...
  app.post('/api/v2/submissions', async (request, reply) => {
    ...
    try {
      ...
      return reply.code(202).send({ apiVersion: 2, id: submissionId, status: 'queued', requiredSchemaVersion: 3 });
    } catch (error) {
  ...
  app.post('/api/v2/plugins/:id/revalidate', async (request, reply) => {
    const auth = await requireUser(request, reply, db, config);
    if (!auth) return;
    const pluginId = (request.params as { id: string }).id;
    const result = await db.query<{
    ...
    if (plugin.publisher_transfer_status === 'pending') {
      return apiError(reply, 409, 'publisher_transfer_pending', 'publisher transfer 尚待管理员审批');
    }
    try {
      const submissionId = await createSubmission(db, auth.user.id, plugin.source_url);
      return reply.code(202).send({ apiVersion: 2, id: submissionId, status: 'queued' });
    } catch (error) {
      return apiError(reply, 400, 'revalidation_rejected', error instanceof Error ? error.message : String(error));
    }
  });

  app.post('/api/v2/plugins/:id/unpublish', async (request, reply) => {
    const auth = await requireUser(request, reply, db, config);
    if (!auth) return;
    const pluginId = (request.params as { id: string }).id;
    const result = await db.query(`
    ...
    return { apiVersion: 2, ok: true };
  });

  app.post('/api/v2/plugins/:id/reports', async (request, reply) => {
    const auth = await requireUser(request, reply, db, config);
    if (!auth) return;
    const pluginId = (request.params as { id: string }).id;
    const body = request.body as { reason?: string; details?: string };
    ...
    return reply.code(201).send({ apiVersion: 2, ok: true });
  });
  ...
  app.post('/api/v2/admin/plugins/:id/unpublish', async (request, reply) => {
    const auth = await requireUser(request, reply, db, config);
    if (!auth || !isAdmin(auth.user, config)) {
      return auth ? apiError(reply, 403, 'admin_required', '需要管理员权限') : undefined;
    }
  ...
  app.post('/api/v2/admin/plugins/:id/restore', async (request, reply) => {
    const auth = await requireUser(request, reply, db, config);
    if (!auth || !isAdmin(auth.user, config)) {
      return auth ? apiError(reply, 403, 'admin_required', '需要管理员权限') : undefined;
    }
    const pluginId = (request.params as { id: string }).id;
    const result = await db.query(`
      UPDATE plugins SET status='published',admin_suspended=false,source_failure_count=0,updated_at=now()
  ...
  app.post('/api/v2/admin/plugins/:id/publisher-transfer/approve', async (request, reply) => {
    const auth = await requireUser(request, reply, db, config);
    if (!auth || !isAdmin(auth.user, config)) {
      return auth ? apiError(reply, 403, 'admin_required', '需要管理员权限') : undefined;
    }
    ...
    const result = await transaction(db, async (client) => {
      const updated = await client.query<{
        ...
        WHERE plugin_id=$1 AND publisher_transfer_status='pending' AND pending_owner_id IS NOT NULL
      ...
      `, [id(), auth.user.id, pluginId, JSON.stringify({
        publisherSubjectId: row.publisher_subject_id,
    ...
    if (!result) return apiError(reply, 409, 'publisher_transfer_not_pending', '没有待审批的 publisher transfer');
    return { apiVersion: 2, ok: true, publisher: result };
  });

  app.post('/api/v2/admin/plugins/:id/publisher-transfer/reject', async (request, reply) => {
    const auth = await requireUser(request, reply, db, config);
    if (!auth || !isAdmin(auth.user, config)) {
      return auth ? apiError(reply, 403, 'admin_required', '需要管理员权限') : undefined;
    }
    ...
    const reason = String((request.body as { reason?: string })?.reason ?? '').trim().slice(0, 500);
    const result = await transaction(db, async (client) => {
      const pending = await client.query<{ pending_owner_id: string; pending_owner_login: string }>(`
        UPDATE plugins
        ...
        RETURNING pending_owner_id,pending_owner_login
      `, [pluginId]);
  ...
  app.post('/api/v2/admin/reports/:id/resolve', async (request, reply) => {
    const auth = await requireUser(request, reply, db, config);
    if (!auth || !isAdmin(auth.user, config)) {
      return auth ? apiError(reply, 403, 'admin_required', '需要管理员权限') : undefined;
    }
