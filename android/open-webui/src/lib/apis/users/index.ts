...
import { getLocalUser, isLocalFirstClient } from '$lib/local-first';
import { getCurrentLocation, supportsNativeAndroidTools } from '$lib/local-first/android-tools';
import { getLocalSettings, saveLocalSettings } from '$lib/local-first/db';
...
export const getUserSettings = async (token: string) => {
	if (isLocalFirstClient()) {
		return { ui: await getLocalSettings() };
	}
...
export const updateUserSettings = async (token: string, settings: object) => {
	if (isLocalFirstClient()) {
		const localSettings = settings as Record<string, any>;
		return saveLocalSettings(localSettings?.ui ?? localSettings);
	}
...
export const updateUserInfo = async (token: string, info: object) => {
	if (isLocalFirstClient()) {
		return { ...getLocalUser(), info: { ...(getLocalUser()?.info ?? {}), ...info } };
	}
...
export const getAndUpdateUserLocation = async (token: string) => {
	const location = await (supportsNativeAndroidTools()
		? getCurrentLocation({ timeoutMs: 15000, highAccuracy: false }).then(
				(position) => position.formatted ?? `${position.latitude}, ${position.longitude}`
			)
		: getUserPosition()
	).catch((err) => {
		console.error(err);
