...
export const createNewChat = async (token: string, chat: object, folderId: string | null) => {
	if (isLocalFirstClient()) {
		return createLocalChat(chat as Record<string, any>, folderId);
	}
...
export const unarchiveAllChats = async (token: string) => {
	if (isLocalFirstClient()) {
		return archiveAllLocalChats(false);
	}
...
export const getChatList = async (
	...
	if (isLocalFirstClient()) {
		const res = await listLocalChats({ page });
		return res.map((chat) => ({
			...chat,
...
export const getArchivedChatList = async (
	...
	if (isLocalFirstClient()) {
		const res = await listLocalChats({ page, archived: true });
		return res.map((chat) => ({
			...chat,
			time_range: getTimeRange(chat.updated_at)
		}));
...
export const getAllChats = async (token: string) => {
	if (isLocalFirstClient()) {
		return listLocalChats({ page: null });
	}
...
export const getChatListBySearchText = async (token: string, text: string, page: number = 1) => {
	if (isLocalFirstClient()) {
		const res = await listLocalChats({ page, search: text });
		return res.map((chat) => ({
			...chat,
			time_range: getTimeRange(chat.updated_at)
		}));
...
export const getAllArchivedChats = async (token: string) => {
	if (isLocalFirstClient()) {
		return listLocalChats({ page: null, archived: true });
	}
...
export const getPinnedChatList = async (token: string = '') => {
	if (isLocalFirstClient()) {
		const res = await listLocalChats({ page: null, pinned: true });
		return res.map((chat) => ({
			...chat,
			time_range: getTimeRange(chat.updated_at)
		}));
...
export const toggleChatPinnedStatusById = async (token: string, id: string) => {
	if (isLocalFirstClient()) {
		return toggleLocalChatPinned(id);
	}
...
export const updateChatFolderIdById = async (token: string, id: string, folderId?: string) => {
	if (isLocalFirstClient()) {
		return updateLocalChatFolder(id, folderId ?? null);
	}
...
export const archiveChatById = async (token: string, id: string) => {
	if (isLocalFirstClient()) {
		const chat = await getLocalChat(id);
		return archiveLocalChat(id, !(chat?.archived ?? false));
	}
...
export const updateChatById = async (token: string, id: string, chat: object) => {
	if (isLocalFirstClient()) {
		return updateLocalChat(id, chat as Record<string, any>);
	}
...
export const addTagById = async (token: string, id: string, tagName: string) => {
	if (isLocalFirstClient()) {
		return addLocalChatTag(id, tagName);
	}
...
export const deleteTagById = async (token: string, id: string, tagName: string) => {
	if (isLocalFirstClient()) {
		return deleteLocalChatTag(id, tagName);
	}
...
export const archiveAllChats = async (token: string) => {
	if (isLocalFirstClient()) {
		return archiveAllLocalChats(true);
	}
