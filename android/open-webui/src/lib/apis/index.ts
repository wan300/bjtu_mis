...
import { getLocalBackendConfig, isLocalFirstClient } from '$lib/local-first';
import { getLocalSettings } from '$lib/local-first/db';
import { resolveDirectConnectionKeys } from '$lib/local-first/secrets';
...
export const getModels = async (
	...
	if (isLocalFirstClient()) {
		base = false;
		connections = connections ?? (await getLocalSettings()).directConnections;
	} else {
	...
	if (connections && !base) {
		...
		if (connections) {
			...
			const OPENAI_API_KEYS = isLocalFirstClient()
				? await resolveDirectConnectionKeys(connections)
				: connections.OPENAI_API_KEYS;
			...
			for (const idx in responses) {
				...
				models = models.map((model) => ({
					...
					info: {
						...
						meta: {
							...(model?.info?.meta ?? {}),
							profile_image_url: model?.info?.meta?.profile_image_url ?? '/static/favicon.png',
							capabilities: {
								...(model?.info?.meta?.capabilities ?? {}),
								vision: model?.info?.meta?.capabilities?.vision ?? true
							}
						}
					}
				}));
...
export const getVersion = async (token: string) => {
	if (isLocalFirstClient()) {
		return { version: 'local-first', deployment_id: 'local-first' };
	}
...
export const getVersionUpdates = async (token: string) => {
	if (isLocalFirstClient()) {
		return { current: 'local-first', latest: 'local-first' };
	}
