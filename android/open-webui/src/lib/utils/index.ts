...
const tryNativeCopyToClipboard = async (text, html = null) => {
	try {
		const { supportsNativeAndroidTools, writeClipboard } = await import('$lib/local-first/android-tools');
		if (!supportsNativeAndroidTools()) {
			return false;
		}

		await writeClipboard({ text: String(text ?? ''), html });
		return true;
	} catch (error) {
		console.error('Native clipboard copy failed:', error);
		return false;
	}
};
...
export const copyToClipboard = async (text, html = null, formatted = false) => {
	if (formatted) {
		...
		if (await tryNativeCopyToClipboard(text, styledHtml)) {
			return true;
		}
		...
		let result = false;
		if (await tryNativeCopyToClipboard(text)) {
			return true;
		}
