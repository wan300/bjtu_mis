<script lang="ts">
	...
	$: modelEditableByOwner =
		model?.preset || model?.info?.base_model_id ? model?.info?.user_id === $user?.id : false;
	$: modelEditableByAdmin =
		!nativeAndroid && !(model?.preset || model?.info?.base_model_id) && $user?.role === 'admin';
	$: showModelEditActions = modelEditableByOwner || modelEditableByAdmin;
</script>
	...
	<div slot="content">
		...
		>
			{#if showModelEditActions}
				...
				{#if $user?.role === 'admin' && !nativeAndroid && model?.owned_by === 'ollama'}
					<button
						type="button"
						class="select-none flex rounded-xl py-1.5 px-3 w-full hover:bg-gray-50 dark:hover:bg-gray-800 transition items-center gap-2"
						on:click={(e) => {
