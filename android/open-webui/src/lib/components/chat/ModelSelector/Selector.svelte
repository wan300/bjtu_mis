				...
				<slot>
						...
						{#if tags && items.filter((item) => !(item.model?.info?.meta?.hidden ?? false)).length > 0}
							...
							>
								...
								>
									...
									{#if !nativeAndroid && items.find((item) => item.model?.direct)}
										<button
											class="min-w-fit outline-none px-1.5 py-0.5 {selectedConnectionType ===
											'direct'
												? ''
					...
					<div class="px-2.5 group relative">
						{#if filteredItems.length === 0}
							{#if items.length === 0 && ($user?.role === 'admin' || localFirst)}
						...
						{#if !(searchValue.trim() in $MODEL_DOWNLOAD_POOL) && searchValue && ollamaVersion && $user?.role === 'admin' && !nativeAndroid}
							<Tooltip
