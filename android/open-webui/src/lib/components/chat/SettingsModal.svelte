	...
	const getAvailableSettings = () => {
		if (localFirst) {
			return allSettings.filter((tab) =>
				['general', 'interface', 'connections', 'data_controls', 'about'].includes(tab.id)
			);
		}

		return allSettings.filter((tab) => {
			if (tab.id === 'connections') {
				return !nativeAndroid && $config?.features?.enable_direct_connections;
			}
		...
		<div class="flex flex-col md:flex-row w-full pt-1 pb-4">
				...
				{#if filteredSettings.length > 0}
					{#each filteredSettings as tabId (tabId)}
						...
						{:else if tabId === 'connections'}
							{#if localFirst || (!nativeAndroid && ($user?.role === 'admin' || ($user?.role === 'user' && $config?.features?.enable_direct_connections)))}
								<button
									role="tab"
			...
			>
				...
				{:else if selectedTab === 'interface'}
					...
					/>
				{:else if selectedTab === 'connections' && (localFirst || !nativeAndroid)}
					<Connections
