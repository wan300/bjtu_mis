<script lang="ts">
	...
	import { FIXED_NATIVE_ANDROID_LOCALE, getLanguages, changeLanguage } from '$lib/i18n';
	import { isNativeAndroid } from '$lib/mobile/platform';
	import { isLocalFirstClient } from '$lib/local-first';
	import { requestNotificationPermission } from '$lib/local-first/android-tools';
	...
	const nativeLocalFirst = nativeAndroid && isLocalFirstClient();
	...
	const toggleNotification = async () => {
		if (nativeLocalFirst) {
			...
			const permission = await requestNotificationPermission().catch((error) => {
				console.error(error);
			...
			if (permission.granted) {
				...
				toast.error($i18n.t('Response notifications cannot be activated.'));
			}
			return;
		}
	...
	onMount(async () => {
		...
		languageLocked = nativeAndroid;

		if (languageLocked) {
			lang = FIXED_NATIVE_ANDROID_LOCALE;
			changeLanguage(FIXED_NATIVE_ANDROID_LOCALE);
		} else {
...
<div class="flex flex-col h-full justify-between text-sm" id="tab-general">
	<div class="  overflow-y-scroll max-h-[28rem] md:max-h-full">
		<div class="">
			<div class=" mb-1 text-sm font-medium">
				{nativeLocalFirst ? $i18n.t('Android Settings') : $i18n.t('WebUI Settings')}
			</div>

			<div class="flex w-full justify-between {nativeLocalFirst ? 'hidden' : ''}">
			...
			{#if !nativeLocalFirst && !languageLocked}
			...
			{/if}
			{#if !nativeLocalFirst && !languageLocked && $i18n.language === 'en-US' && !($config?.license_metadata ?? false)}
				<div
		...
		{#if !nativeLocalFirst && ($user?.role === 'admin' || (($user?.permissions.chat?.controls ?? true) && ($user?.permissions.chat?.system_prompt ?? true)))}
		...
		{#if !nativeLocalFirst && ($user?.role === 'admin' || (($user?.permissions.chat?.controls ?? true) && ($user?.permissions.chat?.params ?? true)))}
	...
	<div class="flex justify-end pt-3 text-sm font-medium {nativeLocalFirst ? 'hidden' : ''}">
		<button
