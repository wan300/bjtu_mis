<script lang="ts">
	...
	import {
		...
		vibrate
	} from '$lib/local-first/android-tools';
	...
	const toggleUserLocation = async () => {
		if (userLocation) {
			const position = await (nativeLocalFirst && supportsNativeAndroidTools()
	...
	const toggleResponseAutoCopy = async () => {
		if (nativeLocalFirst && supportsNativeAndroidTools()) {
			await readClipboardText().catch(() => ({ text: '' }));
	...
	const toggleHapticFeedback = async () => {
		if (hapticFeedback && nativeLocalFirst && supportsNativeAndroidTools()) {
			await vibrate({ durationMs: 15 }).catch((error) => {
				console.error(error);
...
>
	...
	<div class=" space-y-3 overflow-y-scroll max-h-[28rem] md:max-h-full">
		<div>
			<h1 class=" mb-2 text-sm font-medium">{$i18n.t('Android Settings')}</h1>
	...
	<div class=" space-y-3 overflow-y-scroll max-h-[28rem] md:max-h-full">
		<div>
			...
			{#if $user?.role === 'admin' && !nativeAndroid}
				<div>
					<div class=" py-0.5 flex w-full justify-between">
