<script lang="ts">
	...
	import { openTextDocument, saveTextDocument } from '$lib/local-first/android-tools';
	...
	const nativeLocalFirst = localFirst && isNativeAndroid();
	...
	$: if (importFiles) {
		...
		reader.onload = (event) => {
			parseAndImportChats(String(event.target.result ?? ''));
		};
	...
	const parseAndImportChats = async (content: string) => {
	...
	const nativeImportChats = async () => {
		const document = await openTextDocument({
			mimeTypes: ['application/json', 'text/json', 'text/plain']
		}).catch((error) => {
			if (!`${error}`.includes('user_denied')) toast.error(`${error}`);
			return null;
		});

		if (!document?.content) return;
		await parseAndImportChats(String(document.content)).catch((error) => {
			toast.error(`${error}`);
		});
	};
	...
	const exportChats = async () => {
		const content = JSON.stringify(await getAllChats(localStorage.token));
...
<div id="tab-chats" class="flex flex-col h-full justify-between text-sm">
	<div class="space-y-3 overflow-y-scroll max-h-[28rem] md:max-h-full">
		...
		<div>
			...
			{#if !localFirst && !nativeLocalFirst}
				<div>
					<div class="py-0.5 flex w-full justify-between">
