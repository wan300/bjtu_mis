<script lang="ts">
	...
	import { models, settings, isMobileClient } from '$lib/stores';
				...
				>
					...
					{#if !readOnly}
						<Tooltip content={$i18n.t('Edit')} placement="bottom">
							<button
								class="{($settings?.highContrastMode ?? false) || $isMobileClient
									? ''
					...
					{#if message?.content}
						<Tooltip content={$i18n.t('Copy')} placement="bottom">
							<button
								class="{($settings?.highContrastMode ?? false) || $isMobileClient
									? ''
									: 'invisible group-hover:visible'} p-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg dark:hover:text-white hover:text-black transition touch-target"
								on:click={() => {
