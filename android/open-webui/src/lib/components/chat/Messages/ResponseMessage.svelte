<script lang="ts">
	...
	const responseActionButtonClass =
		'inline-flex size-7 shrink-0 items-center justify-center rounded-lg p-1.5 hover:bg-black/5 dark:hover:bg-white/5 dark:hover:text-white hover:text-black transition';
			...
			<div>
				...
				{#if !edit}
					<div
						...
						class="flex items-center justify-start gap-1 overflow-x-auto buttons text-gray-600 dark:text-gray-500 mt-0.5"
					>
						{#if message.done || siblings.length > 1}
							{#if siblings.length > 1}
								<div class="flex items-center gap-0.5 min-w-fit" dir="ltr">
									<button
										aria-label={$i18n.t('Previous message')}
										class={responseActionButtonClass}
										on:click={() => {
							...
							{#if message.done}
									...
									{#if $user?.role === 'user' ? ($user?.permissions?.chat?.edit ?? true) : true}
										<Tooltip content={$i18n.t('Edit')} placement="bottom">
											<button
												...
												$isMobileClient
													? 'visible'
													: 'invisible group-hover:visible'} {responseActionButtonClass}"
												on:click={() => {
													editMessageHandler();
												}}
											>
								...
								<Tooltip content={$i18n.t('Copy')} placement="bottom">
									<button
										...
										class="{isLastMessage || ($settings?.highContrastMode ?? false)
											? 'visible'
											: 'invisible group-hover:visible'} {responseActionButtonClass} copy-response-button"
										on:click={() => {
								...
								{#if !localFirst && !readOnly && ($user?.role === 'admin' || ($user?.permissions?.chat?.tts ?? true))}
								...
								{#if message.usage}
									...
									>
										<button
											...
											class=" {isLastMessage || ($settings?.highContrastMode ?? false)
												? 'visible'
												: 'invisible group-hover:visible'} {responseActionButtonClass} whitespace-pre-wrap"
											on:click={() => {
												console.log(message);
											}}
											id="info-{message.id}"
								...
								{#if !readOnly}
									{#if !localFirst && !$temporaryChatEnabled && ($config?.features.enable_message_rating ?? true) && ($user?.role === 'admin' || ($user?.permissions?.chat?.rate_response ?? true))}
											...
											>
												<Tooltip content={$i18n.t('Regenerate')} placement="bottom">
													<button
														...
														class="{isLastMessage
															? 'visible'
															: 'invisible group-hover:visible'} {responseActionButtonClass} regenerate-response-button"
													>
														<svg
															xmlns="http://www.w3.org/2000/svg"
															fill="none"
