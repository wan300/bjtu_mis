<script lang="ts">
	...
	import { createKeyboardAvoidance } from '$lib/mobile/keyboard-avoidance';
	...
	const triggerHapticFeedback = () => {
		...
		if (supportsNativeAndroidTools()) {
			void nativeVibrate({ durationMs: 5 }).catch((error) => {
				console.error('Native haptic feedback failed:', error);
			});
			return;
		}
	...
	const localReasoningStates = new Map<string, LocalReasoningState>();

	const getReasoningContent = (value: Record<string, any> | null | undefined) =>
	...
	const renderReasoningDetails = (reasoning: string, done: boolean, duration = 0) => {
		const quoted = reasoning
			.trim()
			.split(/\r?\n/)
			.map((line) => (line.startsWith('>') ? line : `> ${line}`))
			.join('\n');
		const durationAttribute = done ? ` duration="${duration}"` : '';

		return `<details type="reasoning" done="${done ? 'true' : 'false'}"${durationAttribute}>\n<summary>${done ? `Thought for ${duration} seconds` : 'Thinking...'}</summary>\n${escapeHtml(quoted)}\n</details>\n\n`;
	};

	const convertReasoningTagsToDetails = (content: string = '') => {
	...
	const updateReasoningMessageContent = (
		message: Record<string, any>,
		state: LocalReasoningState
	) => {
		const prefix = state.prefix.trimEnd();
	...
	const appendReasoningToMessage = (message: Record<string, any>, reasoning: string) => {
		if (!reasoning) {
			return;
		}

		let state = localReasoningStates.get(message.id);
		if (!state) {
			state = {
			...
			localReasoningStates.set(message.id, state);
		}

		state.reasoning += reasoning;
		updateReasoningMessageContent(message, state);
	};

	const appendContentAfterReasoning = (message: Record<string, any>, value: string) => {
		const state = localReasoningStates.get(message.id);
		if (!state) {
			return false;
		}
		...
		if (!state.done) {
			state.done = true;
			state.duration = Math.max(0, Math.round((Date.now() - state.startedAt) / 1000));
		}
	...
	const finalizeReasoningMessage = (message: Record<string, any>) => {
		const state = localReasoningStates.get(message.id);
		...
		if (state) {
			...
			updateReasoningMessageContent(message, state);
			localReasoningStates.delete(message.id);
		}
	...
	const runLocalChatCompletion = async (body, responseMessage, responseMessageId, _chatId) => {
		try {
			...
			if (!res.ok) {
				throw await res.json().catch(() => ({ message: 'Network Problem' }));
			}
	...
	const chatCompletionEventHandler = async (data, message, chatId) => {
		...
		if (status || statuses) {
			const statusItems = [...(Array.isArray(statuses) ? statuses : []), ...(status ? [status] : [])];
			if (message?.statusHistory) {
				message.statusHistory.push(...statusItems);
			} else {
				message.statusHistory = statusItems;
			}
		}
		...
		if (choices) {
			if (choices[0]?.message) {
				// Non-stream response
				const messageReasoning = getReasoningContent(choices[0]?.message);
				const messageContent = choices[0]?.message?.content ?? '';

				if (messageReasoning) {
					message.content += renderReasoningDetails(messageReasoning, true, 0);
				}
				if (messageContent) {
					message.content += convertReasoningTagsToDetails(messageContent);
				}
			} else {
				// Stream response
				...
				if (reasoning) {
					appendReasoningToMessage(message, reasoning);
				}

				if (message.content == '' && value == '\n') {
					console.log('Empty response');
				} else if (value) {
					if (!appendContentAfterReasoning(message, value)) {
	...
	const sendMessage = async (
		...
		if (isLocalFirstClient() && selectedModelIds.length > 1) {
			for (const selectedModelId of selectedModelIds) {
				const model = $models.filter((m) => m.id === selectedModelId).at(0);
				const responseMessageId = messageIdsMap[selectedModelId];
				if (!model || !responseMessageId) {
					continue;
				}

				await sendMessageSocket(
					model,
					messages && messages.length > 0 ? messages : createMessagesList(history, responseMessageId),
					history,
					responseMessageId,
					_chatId
				);
	...
	const sendMessageSocket = async (
		...
		if ($temporaryChatEnabled || isLocalFirstClient()) {
			messages = [
				...messages,
