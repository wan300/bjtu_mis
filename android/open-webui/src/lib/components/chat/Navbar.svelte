...
<nav
	class="sticky top-0 z-30 w-full {chat?.id ? 'pt-0.5 pb-1' : 'pt-1 pb-1'} {$isMobileClient
		? ''
		: '-mb-12'} flex flex-col items-center drag-region"
	style={$isMobileClient
		? `padding-top: calc(max(env(safe-area-inset-top, 0px), var(--safe-area-inset-top, 0px)) + ${
				chat?.id ? '0.125rem' : '0.25rem'
			});`
		: ''}
>
