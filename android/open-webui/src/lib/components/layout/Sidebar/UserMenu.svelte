<script lang="ts">
	...
	import { isAndroid } from '$lib/mobile/platform';
	...
	const DEFAULT_PINNED_ITEMS = localFirst ? [] : ['notes', 'workspace'];
	...
	<div slot="content">
		...
		>
			{#if profile}
				...
				{#if !localFirst && ($user?.status_emoji || $user?.status_message)}
					<div class="mx-1">
						<button
			...
			{#if !$isMobileClient && ($user?.role === 'admin' || $user?.permissions?.workspace?.models || $user?.permissions?.workspace?.knowledge || $user?.permissions?.workspace?.prompts || $user?.permissions?.workspace?.tools)}
			...
			{#if !$isMobileClient && $config?.features?.enable_calendar && ($user?.role === 'admin' || $user?.permissions?.features?.calendar)}
				<div class="flex items-center w-full">
					<a
						href="/calendar"
						draggable="false"
			...
			{#if !$isMobileClient && $config?.features?.enable_automations && ($user?.role === 'admin' || $user?.permissions?.features?.automations)}
				<div class="flex items-center w-full">
					<a
						href="/automations"
						draggable="false"
			...
			{#if !localFirst && showActiveUsers && ($config?.features?.enable_public_active_users_count || role === 'admin') && usage}
				{#if usage?.user_count}
					<hr class=" border-gray-50/30 dark:border-gray-800/30 my-1 p-0" />
