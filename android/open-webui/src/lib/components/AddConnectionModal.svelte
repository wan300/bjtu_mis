<script lang="ts">
	...
	import { LOCAL_PROVIDER_PRESETS } from '$lib/local-first/providers';
	...
	const applyProvider = (providerId: string) => {
		...
		const provider = LOCAL_PROVIDER_PRESETS.find((item) => item.id === providerId);
		if (provider?.baseUrl) {
		...
		if (connection) {
			...
			if (ollama) {
				...
				selectedProvider =
					...
					LOCAL_PROVIDER_PRESETS.find((provider) => provider.baseUrl && provider.baseUrl === url)?.id ??
					'custom';
			}
		}
	};
							...
							<div class="flex gap-2 mt-1.5">
								...
								${($settings?.highContrastMode ?? false) ? 'text-gray-800 dark:text-gray-100' : ''}`}
									...
									<select
										...
										on:change={(event) =>
											applyProvider((event.currentTarget as HTMLSelectElement).value)}
									>
										{#each LOCAL_PROVIDER_PRESETS as provider}
