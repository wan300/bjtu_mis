<script>
	...
	import { executePythonWithWorker } from '$lib/local-first/code-interpreter';
	...
	import { getLocalSettings } from '$lib/local-first/db';
	import { showNotification, supportsNativeAndroidTools } from '$lib/local-first/android-tools';
	import {
		addOpenAIConnection,
	...
	const showSystemNotification = async ({ title, body, icon }) => {
	...
	const executePythonAsWorker = async (id, code, cb, files = []) => {
		const result = await executePythonWithWorker({ id, code, files });
		cb?.({
	...
	onMount(async () => {
		...
		mobile.set(localFirst || window.innerWidth < BREAKPOINT);
		isMobileClient.set(localFirst || (ENABLE_MOBILE_CLIENT && window.innerWidth < BREAKPOINT));

		const onResize = () => {
			if (localFirst) {
				mobile.set(true);
				isMobileClient.set(true);
				return;
			}

			if (window.innerWidth < BREAKPOINT) {
				mobile.set(true);
				isMobileClient.set(ENABLE_MOBILE_CLIENT);
			} else {
				mobile.set(false);
				isMobileClient.set(false);
			}
		};
		...
		window.addEventListener('beforeinstallprompt', (e) => {
			e.preventDefault();
			deferredPrompt = e;
			if (ENABLE_MOBILE_CLIENT) {
				toast.info($i18n.t('Install this app for a better experience'), {
					action: {
		...
		user.subscribe(async (value) => {
			if (value) {
				if (localFirst) {
					settings.set(await getLocalSettings());
		...
		if (localFirst) {
			backendConfig = getLocalBackendConfig();
		} else {
			try {
		...
		const forcedLocale = isNativeAndroid() ? FIXED_NATIVE_ANDROID_LOCALE : undefined;
		initI18n(forcedLocale ?? localStorage?.locale);
		if (forcedLocale) {
			changeLanguage(forcedLocale);
			dayjs.locale(forcedLocale);
		...
		if (backendConfig) {
			...
			if ($config && localFirst) {
				localStorage.setItem('token', localStorage.token || 'local-first-token');
				await settings.set(await getLocalSettings());
				await user.set(ensureLocalSession());
				const localVersion = 'local-first';
				await WEBUI_VERSION.set(localVersion);
				window.WEBUI_VERSION = 'local-first';

				if (['/auth', '/error'].includes($page.url.pathname)) {
					await goto('/');
				}
			} else if ($config) {
