<script lang="ts">
	...
	onMount(async () => {
		...
		if ($isMobileClient) {
			...
			const MOBILE_ALLOWED_PATHS = localFirst
				? ['/', '/c/']
				: ['/', '/c/', '/home', '/channels/', '/notes/'];
			const isAllowed =
				currentPath === '/' ||
				currentPath === '' ||
				MOBILE_ALLOWED_PATHS.some((prefix) => currentPath.startsWith(prefix));
			if (!isAllowed) {
				await goto('/');
				return;
			}
		...
		await Promise.all([
			localFirst ? Promise.resolve() : checkLocalDBChats(),
