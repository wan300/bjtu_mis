<script lang="ts">
	...
	const signUpHandler = async () => {
		...
		if (isCapacitor && !$config?.features.enable_signup) {
			sessionUser = await guestUserSignUp(name, email, password, getDeviceId()).catch(
				(error) => {
					toast.error(`${error}`);
					return null;
				}
...
<div class="w-full h-screen max-h-[100dvh] text-white relative" id="auth-page">
	...
	{#if loaded}
		<div
			...
			id="auth-container"
		>
			<div class="w-full px-10 min-h-screen flex flex-col text-center pb-safe-bottom">
				{#if ($config?.features.auth_trusted_header ?? false) || $config?.features.auth === false}
								...
								<div class="mt-5">
									{#if $config?.features.enable_login_form || $config?.features.enable_ldap || form}
										...
										{:else}
											...
											{#if ($config?.features.enable_signup || isCapacitor) && !($config?.onboarding ?? false)}
