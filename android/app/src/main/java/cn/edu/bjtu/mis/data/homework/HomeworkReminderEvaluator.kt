...
class HomeworkReminderEvaluator(
    private val reminderWindow: Duration = Duration.ofHours(72),
    private val maxDisplayedItems: Int = 3,
) {
    fun evaluate(
    ...
    fun parseDueAt(value: String?): LocalDateTime? {
        return parseHomeworkDueAt(value)
    }
    ...
    private fun candidateFor(item: HomeworkItem, now: LocalDateTime): HomeworkReminderCandidate? {
        ...
        if (remaining.isNegative || remaining > reminderWindow) return null

        return HomeworkReminderCandidate(item = item, dueAt = dueAt, remaining = remaining)
    }

    private fun buildDisplayLines(candidates: List<HomeworkReminderCandidate>): List<String> {
        val displayed = candidates.take(maxDisplayedItems.coerceAtLeast(1))
        val lines = displayed.map {
            "${it.item.course}: ${it.item.title}（截止 ${DueAtLabelFormatter.format(it.dueAt)}）"
        }.toMutableList()
...
class HomeworkReminderCoordinator(
    ...
    private val evaluator: HomeworkReminderEvaluator = HomeworkReminderEvaluator(),
) {
    fun maybeSend(
        items: List<HomeworkItem>,
        now: LocalDateTime = LocalDateTime.now(),
