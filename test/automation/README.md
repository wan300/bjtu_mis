...
这里存放只用于人工采集和离线分析的自动化脚本。脚本会复用本地 Playwright 持久化浏览器 profile，并把网络日志写入 `captures/logs/`。当前项目已经移除旧 backend，脚本内置了采集所需的最小配置。
...
```powershell
python test\automation\cli.py record-mail --start-url "https://mis.bjtu.edu.cn/home/"
```
...
```powershell
python test\automation\cli.py analyze-mail-capture
```

分析指定录制目录：

```powershell
python test\automation\cli.py analyze-mail-capture "d:\code\project\bjtu_web\captures\logs\mail_YYYYMMDD_HHMMSS"
```
